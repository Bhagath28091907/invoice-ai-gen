...
export const generateInvoicePDF = async (
  ...
  formData.items.forEach((item, index) => {
    ...
    pdf.text(item.totalAmount.toFixed(2), margin + colWidths.serial + colWidths.description + colWidths.qty + colWidths.rate + colWidths.gst + 2, yPos + 6, { align: "left" });
    // Items left column
    if (item.itemsLeft) {
  ...
  pdf.text(summary.total.toFixed(2), margin + colWidths.serial + colWidths.description + colWidths.qty + colWidths.rate + colWidths.gst + 2, yPos + 6, { align: "left" });
  yPos += 14;
